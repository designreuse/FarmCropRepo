...
public class SecurityFilter extends UsernamePasswordAuthenticationFilter
    ...
    protected void successfulAuthentication(HttpServletRequest request,
        ...
        request.getSession().setAttribute(USER_FULL_NAME,
                !ObjectUtil.isEmpty(user.getPersonalInfo()) ? user.getPersonalInfo().getName()
                        : "");
        ...
        // request.getSession().setAttribute(USER_OCCUPATION,!ObjectUtil.isEmpty(user.getPersonalInfo())
        // ? user.getPersonalInfo().getOccupation() : "");

        // To Set Preference Data's
        request.getSession().setAttribute(PREF_DATAS, getPrefDatas());

        request.getSession().setAttribute(LANGUAGE_MENU, getLanguageMenu());

        String currentTheme = preferencesService.findPrefernceByName(ESESystem.CURRENT_THEME);
    ...
    public String getLanguageMenu() {
    	...
    	String menuText = "";
		List<Language> languages = clientDAO.listLanguages();
		if (languages.size() > 1) {
			String currentLang = getCurrentLanguage();
			menuText = "<button type='button' data-toggle='dropdown' class='btn header-item waves-effect'>"
					+ getDBProperty("language." + currentLang, currentLang) + " <span class='flag-icon flag-icon-us'></span>"
					+ "</button>" + "<ul class='dropdown-menu' role='menu'>";
			for (Language language : languages) {
				menuText = menuText + "<li><a class='lanMenu' href='auth_executeLang?lang=" + language.getCode() + "'><span class='flag-icon flag-icon-"+language.getFlagCode()+"'></span>"
    ...
    public String getDBProperty(String prop, String language) {
        ...
        String locProp = clientDAO.findLocaleProperty(prop, language);
        return locProp;
    }

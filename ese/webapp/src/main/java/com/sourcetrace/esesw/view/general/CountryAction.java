...
public class CountryAction extends SwitchValidatorAction {
	...
	public JSONObject toJSON(Object obj) {
		...
		if ((getIsMultiBranch().equalsIgnoreCase("1")
			...
			if (StringUtil.isEmpty(branchIdValue)) {
				rows.add(!StringUtil.isEmpty(getBranchesMap().get(getParentBranchMap().get(country.getBranchId())))
						? getBranchesMap().get(getParentBranchMap().get(country.getBranchId()))
						: getBranchesMap().get(country.getBranchId()));
			}
			rows.add(getBranchesMap().get(country.getBranchId()));
		...
		rows.add("<font color=\"#0000FF\" style=\"cursor:pointer;\">" + country.getCode() + "</font>");
		}
	...
	public void populateCountryCreate(){
		...
		country.setCode(idGenerator.getCountryIdSeq());
		country.setBranchId(getBranchId());
		Country c=locationService.findCountryByName(country.getName());
	...
	public void populateCountryUpdate(){
		if (!StringUtil.isEmpty(getId())) {
			country = locationService.findCountryById(Long.valueOf(getId()));
			country.setName(getName());
	...
	public String update() throws Exception {
		...
		if (id != null && !id.equals("")) {
			country = locationService.findCountryById(Long.valueOf(id));
			if (country == null) {
			...
			if (country != null) {
				Country temp = locationService.findCountryById(country.getId());
				if (temp == null) {
					addActionError(NO_RECORD);
					return REDIRECT;
				}
				...
				locationService.editCountry(temp);
			}
	...
	public void delete() throws Exception {
		...
		if (this.getId() != null && !(this.getId().equals(EMPTY))) {
			...
			if (country != null && !ObjectUtil.isListEmpty(country.getStates())) {
				...
				locationService.removeCountry(country.getName());
